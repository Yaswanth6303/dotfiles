...
obj.version = "2"
obj.author =
"ashfinal <ashfinal@gmail.com> and Daniel Marques <danielbmarques@gmail.com> and Omar El-Domeiri <omar@doesnotexist.com> and Daniel German <dmg@turingmachine.org>"
...
function obj:end_of_timer_notify(requestedMinutes)
    ...
    -- when closing window
    if obj.notify then
        hs.dialog.alert(mainRes.x + mainRes.w / 2 - 50, mainRes.y + mainRes.h / 2 - 50, function(result)
            currentWin:focus()
        end, message, strTime, "OK")
    end
end
...
function obj:bar_create_timer()
    ...
    barTimer = hs.timer.doEvery(interval, function()
...
function obj:warning_show(min)
    obj:show_message(string.format(obj.warningFormat, min // 60, min % 60), obj.warningShowDuration)
end

function obj:warning_update()
    if obj.warningShow and obj.timeLeft > 0 then
        minLeft = obj.timeLeft / 60
        if minLeft < obj.timerLenMinutes and minLeft > 0 and obj:is_power_of_2(minLeft) then
    ...
    local items = {
        {
            ...
            fn = function()
                obj:startFor(obj.defaultLenMinutes)
            end,
        },
        {
            title = string.format("%s Start for ... ", obj.menuBarIconActive),
            fn = function()
                obj:startForInteractive()
            end,
        },
    }
...
function obj:create_tick_timer()
    ...
    obj:tick()
    obj.timer = hs.timer.doWhile(function()
        return obj.timerRunning
    end, function()
        obj:tick()
    end, 1)
end
...
function obj:startForInteractive(callback)
    ...
    local currentWin = hs.window.focusedWindow()
    hs.focus()
    local button, time = hs.dialog.textPrompt(
        "Enter time",
        "in minutes or specific time (e.g. 10:30)",
        string.format("%s", obj.defaultLenMinutes),
        "OK",
        "Cancel"
    )
    if button == "OK" then
...
function obj:cancel()
    if obj.timerRunning then
        ...
        obj:reset_timer()
        obj:show_message(string.format("Timer was cancelled (time left %3.1f min).", minLeft))
    else
...
function obj:setProgress(progress)
    if obj.timerRunning then
        ...
        obj:show_message(string.format("Timer reset to %.1f%% (%.1f minutes left)", progress * 100, newMinutesLeft))
        if obj.callback then
